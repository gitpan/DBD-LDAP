...
sub connect
{
	...
	while (<DBFILE>)
	{
		...
		$ldap_tables{$tablename} =~ s/\<root\>/$ldap_root/gi;
	...
	unless ($mesg)
	{
		DBI::set_err($drh, -1, "Could not bind - \"$ldap_hostname\" (".$mesg->code().':'.$mesg->error().")!");
		return undef;
	}
	if ($mesg->code())
	{
		DBI::set_err($drh, -1, "Could not bind to \"$ldap_hostname\" (".$mesg->code().':'.$mesg->error().")!");
		return undef;
	}
